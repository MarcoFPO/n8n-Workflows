...
class ServiceConfig:
    ...
    TIMEFRAMES = {
        "1W": {"display_name": "1 Woche", "description": "Kurzfristige KI-Prognosen für 7 Tage", "days": 7, "icon": "📊", "css_class": "timeframe-week"},
        "1M": {"display_name": "1 Monat", "description": "Mittelfristige Vorhersagen für 30 Tage", "days": 30, "icon": "📈", "css_class": "timeframe-month"},
        "3M": {"display_name": "3 Monate", "description": "Langfristige Prognosen für ein Quartal", "days": 90, "icon": "📊", "css_class": "timeframe-quarter"},
...
@app.get("/prognosen", response_class=HTMLResponse, summary="KI-Prognosen Interface - Direkte Weiterleitung")
async def prognosen(
    timeframe: str = Query(default="1M", description="Standard-Zeitintervall für KI-Prognosen"),
    ...
    """
    KI-Prognosen Handler mit direkter Weiterleitung
    
    Leitet direkt zu einem Standard-Zeitraum weiter, um die Zwischenseite zu vermeiden.
    Standard: 1M (1 Monat) als häufigst genutzter Zeitraum
    """
    
    # Direkte Weiterleitung zu spezifischem Zeitraum
    
    # Verwende den Standard-Zeitraum oder den angegebenen
    redirect_timeframe = timeframe if timeframe in ServiceConfig.TIMEFRAMES else "1M"
    
    # Baue die Redirect-URL
    redirect_url = f"/prognosen/{redirect_timeframe}"
    
    # Füge Navigation Parameters hinzu, falls vorhanden
    if nav_timestamp and nav_direction:
        redirect_url += f"?nav_timestamp={nav_timestamp}&nav_direction={nav_direction}"
    
    # Redirect Response zurückgeben
...
async def prognosen_timeframe(
    ...
    """Prognosen für spezifischen Zeitraum Handler - Mit SOLL-IST Design"""
    ...
    try:
        ...
        def get_prognose_navigation_periods(current_timeframe: str, nav_ts: Optional[int] = None, nav_dir: Optional[str] = None):
        ...
        nav_periods = get_prognose_navigation_periods(timeframe, nav_timestamp, nav_direction)
        
        # Prognose-Daten von Backend abrufen
        prediction_url = ServiceConfig.get_prediction_url(timeframe)
        logger.info(f"Loading predictions from: {prediction_url}")
        
        predictions_data = await http_client.get(prediction_url)
        logger.info(f"Received predictions data: {predictions_data}")
        
        # Generate enhanced predictions table
        if predictions_data and "predictions" in predictions_data:
            enhanced_headers = ['Datum', 'Symbol', 'Unternehmen', 'Gewinn-Prognose', 'Konfidenz', 'Risiko', 'Empfehlung']
            
            table_rows = ""
            prediction_date = nav_periods['current']
            
            # SORTIER-FIX: Sortiere Predictions nach Score/Prediction-Percent (absteigend)
            sorted_predictions = sorted(
                predictions_data["predictions"], 
                key=lambda p: float(p.get('prediction_percent', '0%').replace('%', '')), 
                reverse=True
            )
            
            for prediction in sorted_predictions[:15]:  # Top 15 Prognosen (sortiert)
                symbol = prediction.get('symbol', 'N/A')
                company = prediction.get('company', 'N/A')
                ...
                # Risiko-Bewertung basierend auf Konfidenz
                if confidence >= 80:
                    risk = '<span style="background: #28a745; color: white; padding: 2px 6px; border-radius: 3px;">Niedrig</span>'
                elif confidence >= 60:
                    risk = '<span style="background: #ffc107; color: white; padding: 2px 6px; border-radius: 3px;">Mittel</span>'
                else:
                    risk = '<span style="background: #dc3545; color: white; padding: 2px 6px; border-radius: 3px;">Hoch</span>'
                ...
                rec_color = '#28a745' if 'BUY' in recommendation else '#dc3545' if recommendation == 'SELL' else '#ffc107'
                
                table_rows += f"""
                <tr>
                    <td><strong>{prediction_date}</strong></td>
                    <td><strong>{symbol}</strong></td>
                    <td>{company}</td>
                    <td><span style="color: {profit_color}; font-weight: bold;">{profit:+.2f}%</span></td>
                    <td><span style="color: {confidence_color};">{confidence:.1f}%</span></td>
                    <td>{risk}</td>
                    <td><span style="background: {rec_color}; color: white; padding: 3px 8px; border-radius: 3px; font-size: 0.85em; font-weight: bold;">{recommendation}</span></td>
                </tr>
                """
            ...
            <div class="alert alert-warning">
                ...
                Für den Zeitraum {tf_data['display_name']} sind derzeit keine KI-Prognosen verfügbar.
            </div>
            """
        ...
        content = f"""
        <h2>📊 KI-Prognosen - {tf_data['display_name']}</h2>
        <div class="alert alert-info">
            <p><strong>Zeitraum:</strong> {tf_data['days']} Tage | <strong>Beschreibung:</strong> {tf_data['description']}</p>
            <p><strong>Service:</strong> ML Analytics Service mit Ensemble-Methode (4-Modell Weighted Average)</p>
        </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin: 20px 0; padding: 15px; background: #f8f9fa; border-radius: 8px; border-left: 4px solid #007bff;">
            <button onclick="navigatePrognose('previous', '{timeframe}')" 
            ...
            <button onclick="navigatePrognose('next', '{timeframe}')" 
        ...
        <!-- Zeitintervall-Auswahl wie bei SOLL-IST -->
        <h3>🔧 Zeitintervall auswählen</h3>
        <div class="btn-group">
            <button class="btn {'btn-primary' if timeframe == '1W' else 'btn-outline-primary'}" onclick="loadPrognose('1W')">📅 1 Woche</button>
            <button class="btn {'btn-primary' if timeframe == '1M' else 'btn-outline-primary'}" onclick="loadPrognose('1M')">📆 1 Monat</button>
            <button class="btn {'btn-primary' if timeframe == '3M' else 'btn-outline-primary'}" onclick="loadPrognose('3M')">🗓️ 3 Monate</button>
            <button class="btn {'btn-primary' if timeframe == '12M' else 'btn-outline-primary'}" onclick="loadPrognose('12M')">📊 1 Jahr</button>
        </div>
        
        <!-- KI-Prognosen Tabelle mit Ladezeit -->
        <h3>📈 Top KI-Prognosen - {tf_data['display_name']} <small>(Ladezeit: {load_time:.2f}s)</small></h3>
        {table_html}
        
        <div class="alert alert-success">
            <h3>🤖 KI-Prognosen Erklärung</h3>
            <ul>
                <li><strong>Gewinn-Prognose:</strong> ML-basierte Vorhersage der erwarteten Rendite</li>
                <li><strong>Konfidenz:</strong> Vertrauenswürdigkeit der Prognose (0-100%)</li>
                <li><strong>Risiko:</strong> Basierend auf Konfidenz-Score (Niedrig/Mittel/Hoch)</li>
                <li><strong>Empfehlung:</strong> Algorithmische Handelsempfehlung (BUY/HOLD/SELL)</li>
            </ul>
        </div>
        
        <script>
            function navigatePrognose(direction, timeframe) {{
                ...
                // Refresh page with navigation parameters for KI-Prognosen
            ...
            function loadPrognose(newTimeframe) {{
                // Navigate to different timeframe
                window.location.href = '/prognosen/' + newTimeframe;
            }}
        </script>
        
        <div style="margin-top: 30px;">
            <a href="/prognosen" style="background: #3498db; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">⬅️ Zurück zur Übersicht</a>
        </div>
        """
        ...
        def get_error_navigation_periods(current_timeframe: str, nav_ts: Optional[int] = None, nav_dir: Optional[str] = None):
            from datetime import datetime, timedelta
            timeframe_deltas = {"1W": timedelta(weeks=1), "1M": timedelta(days=30), "3M": timedelta(days=90), "12M": timedelta(days=365)}
        ...
        <div class="alert alert-error">
            ...
            <p>Die KI-Prognosen können derzeit nicht abgerufen werden.</p>
            <p><strong>Error:</strong> {str(e)}</p>
            <p><strong>Service URL:</strong> {prediction_url if 'prediction_url' in locals() else 'N/A'}</p>
        </div>
        ...
        <script>
            function navigatePrognose(direction, timeframe) {{
                var timeframeDays = {{ '1W': 7, '1M': 30, '3M': 90, '12M': 365 }};
                var daysToAdd = direction === 'next' ? timeframeDays[timeframe] : -timeframeDays[timeframe];
                var newDate = new Date();
                newDate.setDate(newDate.getDate() + daysToAdd);
                var timestamp = Math.floor(newDate.getTime() / 1000);
    ...
    return HTMLTemplateService.generate_base_template(f"KI-Prognosen {tf_data['display_name']}", content)
...
async def vergleichsanalyse(
    ...
    try:
        ...
        logger.info(f"DEBUG: comparison_list type: {type(comparison_list)}, length: {len(comparison_list) if comparison_list else 0}")
        ...
        if comparison_list and isinstance(comparison_list, list) and len(comparison_list) > 0:
            ...
            for item in comparison_list[:15]:  # Top 15 Vergleiche
                ...
                soll_performance = item.get('soll_value', 0)  # Korrigiert: soll_value statt soll_performance
                ist_performance = item.get('ist_value', 0) or 0  # Korrigiert: ist_value statt ist_performance  
                deviation = item.get('deviation_percentage', 0) or 0  # Korrigiert: deviation_percentage statt deviation
