...
async def _update_device(host: str) -> tuple[str, bool]:
    ...
    try:
        async with asyncssh.connect(
            ...
            # MikroTik unterstützt ältere Algorithmen — alle zulassen
            encryption_algs=asyncssh.encryption_algs,
            kex_algs=asyncssh.kex_algs,
            server_host_key_algs=asyncssh.server_host_key_algs,
        ) as conn:
...
async def mikrotik_update_job() -> None:
    ...
    for i, host in enumerate(MIKROTIK_HOSTS):
        ...
        if i < len(MIKROTIK_HOSTS) - 1:
            # Pause damit das Gerät Zeit zum Neustarten hat
            await asyncio.sleep(_INTER_DEVICE_PAUSE)

    ok_count = sum(1 for _, ok in results if ok)
    fail_count = len(results) - ok_count
    lines = "\n".join(
        f"{'✅' if ok else '❌'} {text}" for text, ok in results
    )
    summary = (
    ...
    logger.info(
        "mikrotik_update.done ok=%d fail=%d", ok_count, fail_count
    )
