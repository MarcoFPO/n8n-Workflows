from sqlalchemy import Column, Integer, String, DateTime, Decimal, Text, Boolean, Date, ForeignKey, JSON, ARRAY, UniqueConstraint
...
class User(Base):
    ...
    ml_training_data = relationship("MLTrainingData", back_populates="user")
...
class AldiProduct(Base):
    ...
    nutrition_per_100g = Column(JSON)  # Kalorien, Protein, Kohlenhydrate, Fett, etc.
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
...
class Inventory(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    product_id = Column(Integer, ForeignKey("aldi_products.id"))
...
class Recipe(Base):
    ...
    prep_time = Column(Integer)  # Minuten
    cook_time = Column(Integer)  # Minuten
    ...
    ml_training_data = relationship("MLTrainingData", back_populates="recipe")
...
class MealLog(Base):
    ...
    rating = Column(Integer)  # 1-5 Sterne
    notes = Column(Text)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class RecipeRating(Base):
    ...
    complexity_rating = Column(Integer)  # 1-5 (wie schwer war es?)
    satisfaction_rating = Column(Integer)  # 1-5 (wie satt/zufrieden?)
    health_impact_rating = Column(Integer)  # 1-5 (wie gesund gefühlt?)
...
class MLTrainingData(Base):
    ...
    target_score = Column(Decimal(3, 2))  # Kombinierter Score aus Ratings
    weight = Column(Decimal(5, 4), default=1.0)  # Gewichtung für Training
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    
    # Relationships
    user = relationship("User", back_populates="ml_training_data")
    recipe = relationship("Recipe", back_populates="ml_training_data")
...
class ReceiptScan(Base):
    ...
    processed_items = Column(JSON)  # Array von erkannten Produkten
